Vue.component("registracija-prodavca", {
  data: function () {
    return {
      form: {
        username: "",
    ...
    <div>
      ...
      <b-form-group id="input-group-3" label="Pol:" label-for="input-3">
        ...
        <input type = "date" id="input-5" v-model="form.birthDate" required></input>
         ...
         <b-button type="submit" variant="primary">Registruj radnika</b-button>
        </b-form>
  ...
  methods: {
    onSubmit() {
      ...
      axios
        .post(`rest/user/registerWorker`, this.form)
        .then((response) => {
          this.$router.push("/");
        })
        .catch((error) => {
          console.log("Greska.");
          alert("Postoji radnik sa unetim imenom");
        });
    },
    onReset() {},
  },
